...
def generate_neg_samples(raw_dataset, sample_per_img=3, crop_size=(48, 48)):
    '''
...
pos_samples = generate_pos_samples(raw_dataset, sample_per_img=3, pad_range=(15, 150), img_size=input_dim[0])
...
plt.imshow(pos_samples[0][0])
plt.savefig('sample_pos.png')

plt.imshow(neg_samples[0][0])
plt.savefig('sample_neg.png')
...
rnet = build_rnet_model(batch_norm=True, dropout=True) # build_rnet_model(input_shape=input_dim, batch_norm=False, dropout=True)
...
def get_custom_bbox_regression_loss(reduction='sum', batch_size=16):
    def custom_bbox_regression_loss(y_true, y_pred, reduction=reduction, batch_size=batch_size):
        ...
        # log_loss = reduction((w_gt - w_pr) ** 2) + reduction((h_gt - h_pr)**2)
...
history = rnet.fit(train_dataset, epochs=epochs, 
...
# rnet.compile(optimizer=Adam(lr=0.000001, amsgrad=True),
#             loss=losses_2,
#             loss_weights=loss_weights,
#             metrics={'probability':'accuracy'})

# history = rnet.fit(train_dataset, initial_epoch=epochs//10,
